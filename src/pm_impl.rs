...
impl<'a> midi::MidiInterface<'a> for MidiImpl {
    ...
    fn get_input(&'a self, identifier: Identifier) -> Result<InputPort<'a>, MidiInterfaceError> {
        let devs: Vec<pm::DeviceInfo> = self.iface.devices()?;
        ...
        Ok(self.iface.input_port(input, BUFFER_SIZE)?)
    }
    ...
    fn get_output(&'a self, identifier: Identifier) -> Result<OutputPort<'a>, MidiInterfaceError> {
        ...
        Ok(self.iface.output_port(output, BUFFER_SIZE)?)
    }
    ...
    fn get_in_out(
        ...
        let in_p = self.iface.input_port(in_p, BUFFER_SIZE)?;
        let out_p = self.iface.output_port(out_p, BUFFER_SIZE)?;

        Ok((in_p, out_p))
    }

    fn get_default_input(&'a self) -> Result<Self::MidiInput, MidiInterfaceError> {
        let id = self.iface.default_input_device_id()?;
        let dev = self.iface.device(id)?;
        let dev = self.iface.input_port(dev, BUFFER_SIZE)?;
        Ok(dev)
    }

    fn get_default_output(&'a self) -> Result<Self::MidiOutput, MidiInterfaceError> {
        let id = self.iface.default_output_device_id()?;
        let dev = self.iface.device(id)?;
        let dev = self.iface.output_port(dev, BUFFER_SIZE)?;
        Ok(dev)
    }
}
